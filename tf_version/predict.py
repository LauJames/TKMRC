...
def get_predict_set(input_file):
    wait_answer = []
    with open(input_file, 'r') as fin:
        while True:
            json_line = fin.readline()
            if not json_line:
                print("predict set load successfully!")
                return wait_answer
            wait_answer.append(json_line)


para_question_path = '../data/TKData/wait_answer.json'
predictions_path = '../data/TKData/bidafPredict.json'
...
if __name__ == '__main__':
    ...
    wait_predicts = get_predict_set(para_question_path)
    predictions = []
    for temp in wait_predicts:
        json_obj = json.loads(temp)
        idx = json_obj["question_id"]
        question = json_obj["question"]
        _, reference, answer = infer(question, vocab, search, ir_config, args, rc_model)
        json_str = {
            "id": idx,
        ...
        predictions.append(json_str)

    # write predictions
    with open(predictions_path, 'w') as fout:
        for prediction in predictions:
            fout.write(str(json.dumps(prediction, ensure_ascii=False)) + '\n')
